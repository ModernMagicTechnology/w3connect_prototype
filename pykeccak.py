...
def rol(value, left, bits):
    """
    Circularly rotate 'value' to the left, treating it as a quantity of the given size in bits.
    """
    top = value >> (bits - left)
...
def ror(value, right, bits):
    """
    Circularly rotate 'value' to the right, treating it as a quantity of the given size in bits.
    """
    top = value >> right
...
def keccak_f(state):
    """
    This is Keccak-f permutation. It operates on and mutates the passed-in KeccakState.
    It returns nothing.
    """
...
class KeccakSponge:
    ...
    def absorb_final(self):
        ...
        while len(padded) > 0:
            self.absorb_block(padded[:self.state.bitrate_bytes])
            padded = padded[self.state.bitrate_bytes:]
